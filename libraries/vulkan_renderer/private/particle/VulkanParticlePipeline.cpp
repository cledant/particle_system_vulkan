...
void
VulkanParticlePipeline::setParticleMass(float mass)
{
    _compUbo.particleMass = mass;
}
...
void
VulkanParticlePipeline::acquireComputeBufferBarrier(
  VkCommandBuffer &cmdBuffer) const
{
    if (_queues.graphicQueueIndex == _queues.computeQueueIndex) {
        return;
    }
...
void
VulkanParticlePipeline::releaseComputeBufferBarrier(
  VkCommandBuffer &cmdBuffer) const
{
    if (_queues.graphicQueueIndex == _queues.computeQueueIndex) {
        return;
