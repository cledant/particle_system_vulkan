...
void
VulkanParticleDebugRenderPass::init(VulkanInstance const &vkInstance,
...
void
VulkanParticleDebugRenderPass::resize(VulkanSwapChain const &swapChain)
{
    clear();
    _create_render_pass(swapChain);
    _create_depth_resources(swapChain);
...
void
VulkanParticleDebugRenderPass::clear()
{
    size_t i = 0;
...
void
VulkanParticleDebugRenderPass::_create_render_pass(
  VulkanSwapChain const &swapChain)
{
    // Color
    ...
    if (vkCreateRenderPass(_device, &render_pass_info, nullptr, &renderPass) !=
        ...
        throw std::runtime_error(
          "VulkanParticleDebugRenderPass: failed to create render pass");
    }
}

void
VulkanParticleDebugRenderPass::_create_depth_resources(
  VulkanSwapChain const &swapChain)
{
...
void
VulkanParticleDebugRenderPass::_create_framebuffers(
  VulkanSwapChain const &swapChain)
{
    ...
    for (auto const &it : swapChain.swapChainImageViews) {
        ...
        if (vkCreateFramebuffer(
            ...
            VK_SUCCESS) {
            throw std::runtime_error(
              "VulkanParticleDebugRenderPass: Failed to create framebuffer");
        }
        ++i;
    }
}